...
var superagentPromise = _interopDefault(require('superagent-promise'));
...
var tryAll = function tryAll(promiseConstructors) {
  var promise = new rsvp.Promise(promiseConstructors[0]);
  ...
  function WebSocketPlus(getUrls, protocol) {
    ...
    if (typeof getUrls !== 'function') {
      _this._getUrls = function () {
        return rsvp.Promise.resolve(getUrls);
      };
    } else {
...
var debug$6 = d('LC:ConversationQuery');
...
var IMClient = function (_Client) {
  ...
  IMClient.prototype._dispatchSessionMessage = function _dispatchSessionMessage(message) {
    ...
    switch (message.op) {
      ...
      default:
        this.emit('unhandledmessage', message);
        return rsvp.Promise.reject(new Error('Unrecognized session command'));
    }
  };
  ...
  IMClient.prototype._dispatchConvMessage = function _dispatchConvMessage(message) {
    ...
    switch (message.op) {
      ...
      default:
        this.emit('unhandledmessage', message);
        return rsvp.Promise.reject(new Error('Unrecognized conversation command'));
    }
  };
  ...
  IMClient.prototype._sendAck = function _sendAck(message) {
    ...
    if (!cid) {
      return rsvp.Promise.reject(new Error('missing cid'));
    }
  ...
  IMClient.prototype._doSendAck = function _doSendAck() {
    ...
    return rsvp.Promise.all(_Object$keys(this._ackMessageBuffer).map(function (cid) {
  ...
  IMClient.prototype._open = function _open(appId, tag, deviceId) {
    ...
    return rsvp.Promise.resolve(new GenericCommand({
      cmd: 'session',
      op: 'open',
      appId: appId,
  ...
  IMClient.prototype.getConversation = function getConversation(id) {
    ...
    if (cachedConversation) {
      return rsvp.Promise.resolve(cachedConversation);
    }
...
var debug$7 = d('LC:MessageParser');
...
var agent = superagentPromise(superagent, rsvp.Promise);
...
var Realtime = function (_EventEmitter) {
  ...
  Realtime.prototype._open = function _open() {
    ...
    this._openPromise = new rsvp.Promise(function (resolve, reject) {
  ...
  Realtime.prototype._getEndpoints = function _getEndpoints(options) {
    ...
    return rsvp.Promise.resolve(this._cache.get('endpoints') || this.constructor._fetchEndpointsInfo(options).then(tap(function (info) {
  ...
  Realtime.prototype._dispatchMessage = function _dispatchMessage(message) {
    if (message.peerId !== null) {
      ...
      if (client) {
        return rsvp.Promise.resolve(client._dispatchMessage(message)).catch(debug);
      }
  ...
  Realtime.prototype.createIMClient = function createIMClient(id, clientOptions, tag) {
    ...
    if (id) {
      if (this._clients[id] !== undefined) {
        return rsvp.Promise.reject(new Error('IMClient[' + id + '] is already created'));
      }
